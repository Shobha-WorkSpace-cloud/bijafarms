...
export default function AnimalTracker() {
  ...
  const handleExportAnimals = () => {
    try {
      ...
      link.setAttribute("download", `bija-animals-${new Date().toISOString().split("T")[0]}.csv`);
      ...
      toast({
        ...
        description: `Exported ${filteredAnimals.length} animals to CSV file`,
      });
    } catch (error) {
      console.error("Error exporting animals:", error);
      toast({
        title: "Export Failed",
        description: "Failed to export animals. Please try again.",
        variant: "destructive",
      });
    }
  };
